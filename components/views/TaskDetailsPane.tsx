...
const PropertyRow = ({ icon: Icon, label, children }: { icon: any, label: string, children: React.ReactNode }) => (
  <div className="flex items-start sm:items-center min-h-[40px] group hover:bg-zinc-50 dark:hover:bg-zinc-900/30 -mx-2 px-2 py-0.5 sm:py-0 rounded transition-colors">
    ...
    {/* Removed overflow-hidden here to allow DatePicker to pop out */}
...
const EditorToolbar = ({ editor }: { editor: any }) => {
  ...
  return (
    <div className="flex items-center gap-1 mb-3 overflow-x-auto hide-scrollbar pb-1">
  ...
  return (
    <>
      <style>{`
        ...
        /* CUTESY PASTEL CHECKBOXES OVERRIDE */
        ...
        .tiptap input[type="checkbox"]:checked {
          background-color: #f472b6; /* pastel pink */
          border-color: #f472b6;
        }
      ...
      <div 
        ...
        // Using h-[100dvh] fixes mobile safari height shifting during scroll!
        className={`fixed top-0 right-0 h-[100dvh] w-full sm:w-[540px] bg-[var(--background)] sm:border-l border-[var(--border)] z-40 flex flex-col transition-all duration-[400ms] ease-[cubic-bezier(0.32,0.72,0,1)] transform max-w-full ${
      ...
      >
        ...
        {(!displayTaskId || task === undefined) ? (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 sm:px-10 py-10 space-y-6 sm:space-y-8 pb-32 max-w-full">
            ...
            <div className="flex flex-col gap-1 sm:gap-2 text-[15px] max-w-full">
              ...
              {/* Removed overflow clipping wrapper on PropertyRow to allow absolute calendars to pop out */}
            ...
            <div className="space-y-2 pb-4 max-w-full">
              {/* Toolbar restored for easy mobile access, but scrollable so it's compact */}
        ...
        {/* Action Pills: Disappear if keyboard is open to save space */}
        <div className={`absolute bottom-6 left-0 w-full px-6 sm:px-10 flex justify-between items-center z-50 pointer-events-none transition-all duration-300 ease-out ${
