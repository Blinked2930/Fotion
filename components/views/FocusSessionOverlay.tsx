  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[var(--background)]/95 backdrop-blur-2xl animate-in fade-in duration-300 flex flex-col">
      ...
      <div className="flex items-center justify-between p-6">
        <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold tracking-widest uppercase text-sm">
          <Target className="w-5 h-5" /> Focus Session
        </div>
        <button 
          onClick={onClose} 
      ...
      <div className="flex-1 flex flex-col md:flex-row max-w-6xl w-full mx-auto p-6 gap-12 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-center items-center h-full">
          ...
          <div className="flex items-center bg-zinc-100 dark:bg-zinc-900 rounded-full p-1 mb-8 shadow-inner border border-[var(--border)]">
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-bold transition-all ${mode === "work" ? 'bg-white dark:bg-zinc-800 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-zinc-500 hover:text-[var(--foreground)]'}`}
            >
              <Target className="w-4 h-4" /> Deep Work
            </button>
            <button 
              onClick={() => switchMode("break")}
              className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-bold transition-all ${mode === "break" ? 'bg-white dark:bg-zinc-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-zinc-500 hover:text-[var(--foreground)]'}`}
            >
          ...
          <div className="flex items-center gap-4 mb-16">
            <button 
              onClick={toggleTimer}
              className={`w-16 h-16 rounded-full flex items-center justify-center transition-transform active:scale-95 shadow-xl ${isRunning ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400' : 'bg-[var(--foreground)] text-[var(--background)]'}`}
            >
          ...
          {activeTask ? (
            <div className="flex flex-col items-center text-center animate-in slide-in-from-bottom-4 fade-in">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full font-bold shadow-lg shadow-emerald-500/20 transition-transform active:scale-95"
              >
        ...
        <div className="w-full md:w-[400px] h-full flex flex-col border-l border-[var(--border)] md:pl-12 pt-8 md:pt-0">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="px-2.5 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 rounded-md text-xs font-bold">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-24">
            {focusedTasks.length === 0 ? (
              <div className="text-zinc-500 text-sm text-center py-10 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl border border-dashed border-[var(--border)]">
              ...
              focusedTasks.map(task => (
                <div 
                  ...
                  className={`p-4 rounded-xl cursor-pointer transition-all border ${activeTaskId === task._id ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800 shadow-sm' : 'bg-white dark:bg-[#1c1c1c] border-[var(--border)] hover:border-emerald-200 dark:hover:border-emerald-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/50'}`}
                >
                  <div className="flex items-start gap-3">
                    <button 
                      ...
                      className="mt-0.5 w-5 h-5 shrink-0 rounded border border-zinc-300 dark:border-zinc-600 hover:border-emerald-500 hover:bg-emerald-50 flex items-center justify-center transition-colors bg-white dark:bg-transparent"
                    />
                    <span className={`text-[15px] font-medium leading-snug ${activeTaskId === task._id ? 'text-emerald-900 dark:text-emerald-100' : 'text-[var(--foreground)]'}`}>
                      {task.title}
                    </span>
                  </div>
                </div>
